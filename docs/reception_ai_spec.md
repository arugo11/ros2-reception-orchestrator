...
来訪者が受付に話しかけると, ASRが発話を認識し, 対話担当LLMが自然な返答を生成し, TTSが音声として再生する.
並行して, 監視, 判断担当LLMが会話全体を観測し, 情報不足の判定, Discordスレッド作成, スレッド更新, 秘書課からの返信の取り込み, 追加質問の要否判断を行う.
...
- 対話担当LLM呼び出し: 既存 `ros2_vllm` の `/llm/chat` または `/llm/stream/request` を利用
...
- 対話担当LLMと監視, 判断担当LLMを同一呼び出し経路に乗せて, 音声応答をブロックすること
...
flowchart LR
    ...
    ORCH -->|dialog request| A[対話担当LLM]
    A --> ORCH
    ORCH --> TTS[tts_server]
    ORCH -->|observe state| B[監視, 判断担当LLM]
    B --> ORCH
...
- ASR結果を受けて, 丁寧で愛想のよい受付口調の返答文を生成する
- 必要に応じて, 名前, 所属, 来訪目的を自然に聞き返す
- 監視, 判断担当LLMまたは状態機械が決めた「次に聞くべきこと」を反映して応答する
- 秘書課から到着した返信を, 来訪者向け自然文にほぼそのまま変換して返答する
- 確認フェーズでは, 取得済み項目を復唱確認する

Aは, Discord送信, スレッド作成, 状態更新, 重要フラグ判定などのツール実行責務を持たない.

### 7.2 監視, 判断担当LLM, B

Bは, 対話全体をオブザーブする補助エージェントである.
責務は以下とする.

- 会話履歴と抽出済み情報から, 必須項目の充足状況を判断する
- Discordスレッド作成の要否を判断する
- Discordスレッドへ送る更新内容を構造化データとして生成する
- 秘書課への追加確認が必要かを判断する
- 次に対話担当LLMへ渡す質問意図を返す
- 必要に応じて固定の関数呼び出しを行う

Bは「音声応答の主経路」をブロックしてはならない. Bの失敗時でも, 受付として最低限の会話は継続可能でなければならない.
...
対話担当LLMの出力スタイルは, 丁寧, 愛想がよい, 受付らしい, 短く明快, を基本とする.
冗長な説明は避ける.
